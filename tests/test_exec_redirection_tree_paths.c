...
Test(exec_redirection, input_missing_returns_1)
{
    ...
    cr_assert_eq(exec_cmd_with_redirections(node, &env, NULL, NULL), 1);
    free_tree(node);
}
...
Test(exec_redirection, heredoc_fd_prepared_path)
{
    ...
    cr_assert_eq(exec_cmd_with_redirections(node, &env, NULL, NULL), 0);
...
Test(exec_tree_paths, cmd_and_unknown)
{
    ...
    cr_assert_eq(exec_tree(cmd, &env, NULL, NULL), 0);
    cr_assert_eq(exec_tree(unknown, &env, NULL, NULL), 1);
    free_tree(cmd);
    free_tree(unknown);
}
...
Test(exec_tree_paths, and_or_sequence_paths)
{
    ...
    cr_assert_eq(exec_tree(and_node, &env, NULL, NULL), 1);
    or_node->left = new_node(TOKEN_EOF);
    or_node->right = make_cmd_node("env");
    cr_assert_eq(exec_tree(or_node, &env, NULL, NULL), 0);
    seq_node->left = new_node(TOKEN_EOF);
    seq_node->right = new_node(TOKEN_EOF);
    cr_assert_eq(exec_tree(seq_node, &env, NULL, NULL), 1);
    free_tree(and_node);
    free_tree(or_node);
    free_tree(seq_node);
}
