...
Test(exec_pipe_injected, success_last_exitstatus)
{
    ...
    g_wait_status[0] = 0;
    g_wait_status[1] = 0;
    cr_assert_eq(tested_exec_pipe_injected(tree, &env, NULL, NULL), 0);
    free_tree(tree);
}
...
Test(exec_pipe_injected, success_last_signaled)
{
    ...
    cr_assert_eq(tested_exec_pipe_injected(tree, &env, NULL, NULL), 1);
    free_tree(tree);
}
...
Test(exec_pipe_injected, spawn_failure_triggers_cleanup)
{
    ...
    g_fork_ret[0] = 111;
    g_fork_ret[1] = -1;
    cr_assert_eq(tested_exec_pipe_injected(tree, &env, NULL, NULL), 84);
